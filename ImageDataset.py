...
def create_dataset(cfg):
   ...
   # print(len(list_ds))
    ...
    ytest=[]
    ### test, can be removed
    for image, label in validation_labeled_ds:
        xtest.append(image.numpy())
        ytest.append(label.numpy())
    print(len(xtest))
    print(len(ytest))
    print(len(validation_labeled_ds))
    print(type(validation_labeled_ds))
    
    validation_ds = prepare_for_training(cfg, validation_labeled_ds)
    
    #print(list_ds)
    #print(labeled_ds)
   # print(validation_list_ds)
    #print(validation_labeled_ds)

    return train_ds, validation_ds

def create_datasetnew(cfg):
    list_ds = tf.data.Dataset.list_files(str(cfg.training_dir/'*/*'))
    # Set `num_parallel_calls` so multiple images are loaded/processed in parallel.
    cfg_process_path = functools.partial(process_path, cfg)
    train_labeled_ds = list_ds.map(cfg_process_path, num_parallel_calls=cfg.AUTOTUNE)
    #labeled_ds = list_ds.map(process_path, num_parallel_calls=cfg.AUTOTUNE)
    #xtrain=[]
    #ytrain=[]
    ### test, can be removed
    #for image, label in train_labeled_ds:
    #    xtrain.append(image.numpy())
    #    ytrain.append(label.numpy())
        #print("Image shape: ", image.numpy())
        #print("Label: ", label.numpy())
    
    train_ds = prepare_for_training(cfg, train_labeled_ds)
   ...
   # print(len(validation_labeled_ds))
    ...
    validation_ds = prepare_for_training(cfg, validation_labeled_ds)
    return train_labeled_ds, validation_labeled_ds